...
# read in the bbl file data
with open(f'{paper_name}.bbl', 'r') as f:
    bbl_data = f.read()
...
# f = open("sample_bbl.txt", "a")
# for entry in entries[1:4]:  # ignore the first empty entry
#     f.write(entry)
#     f.write("\n")
...
for entry in entries[1:]:  # ignore the first empty entry
    ...
    # pattern_1 = r'\[\{?(.+)(\((\d{4})\))(.*)\}?\][%\s]*?\{([A-Za-z0-9:_-]+)\}[\r\s]*(.+)([\d]{4})'

    # pattern for ACL, ACM, Arxiv
    pattern_1_authors = r'\[\{?(.+)(\((\d{4})\))(.*)\}?\][%\s]*?\{([\/A-Za-z0-9:_-]+)\}[\r\s]*(.+)([\d]{4})'
    # pattern_3 = r'.*\{([A-Za-z0-9\s\r.,:()]+)(\(.*:(.*)\))?\}(,\s*(pages\s*|[\d]+:)([\d-]+)|.)'
    # pattern containing the publication details
    pattern_3_journal = r'(\\emph{|{\\em )([A-Za-z\s0-9.\(\):&\\]*)}'
    pattern_page = r'(pages?}?\s*{?|\d{1,}:)(\d{1,})([–-]{1,}(\d{1,}))?'
    pattern_2_journal = r'''\\bibinfo{booktitle}{\\emph{([A-Za-z0-9\s{}(),".':\--]*)}}'''
    ...
    matched_data_1 = re.findall(pattern_1_authors, data_1)
    if len(matched_data_1) == 0:
        file_type = 'IEEE'
        print("No match found for authors")
    print(matched_data_1)
    print("====================================")

    matched_data_journal = re.findall(pattern_3_journal, data_3)
    if len(matched_data_journal) == 0:
        # print("No match found for journal details")
    
    # if journal not found in third field, then it is in the second field with the field as booktitle
    # create a regular expression on the second field to extract the journal details

        matched_data_journal = re.findall(pattern_2_journal, data_2)
        if len(matched_data_journal) == 0:
            print("journal details not avaiable")
    print(matched_data_journal)
    print("====================================")

    matched_data_page = re.findall(pattern_page, data_3)
    if len(matched_data_page) == 0:
        print("No match found for page details")
    print(matched_data_page)
    print("====================================")
