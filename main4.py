...
def get_information4(root, Matched_data, paper_name, data_1):
    information=init_dict()

    # Author-------------------------------------------
    try:
        if len(Matched_data['21']):
            information['author'] = Matched_data['22'][0]
    ...
    # Title---------------------------------------------
    try:
        if len(Matched_data['22']):
            information['title'] = Matched_data['22'][0]
            # title_elem = ET.SubElement(entry_elem, 'title')
            # title_elem.text = information['title']
    except Exception as e:
        print("Error in fetching title")
    ...
    # Journal-------------------------------------------
    try:
        if len(Matched_data['23']):
            information['journal'] = Matched_data['23'][0]
            # journal_elem = ET.SubElement(entry_elem, 'journal')
            # journal_elem.text = information['journal']
    except Exception as e:
        print("Error in fetching journal")


    # Volume-------------------------------------------
    try:
        if len(Matched_data['24']):

            information['volume'] = Matched_data['24'][0]
            # volume_elem = ET.SubElement(entry_elem, 'volume')
            # volume_elem.text = information['volume']
    except Exception as e:
        print("Error in fetching volume")
    ...
    # Page-------------------------------------------
    try:
        if len(Matched_data['25']):
            information['pages'] = Matched_data['25'][0]
            # pages_elem = ET.SubElement(entry_elem, 'pages')
            # pages_elem.text = information['pages']
    except Exception as e:
        print("Error in fetching pages")
    ...
    # Year-------------------------------------------
    try:
        if len(Matched_data['26']):
            information['year'] = Matched_data['26'][0]
            # year_elem = ET.SubElement(entry_elem, 'year')
            # year_elem.text = information['year']
    except Exception as e:
        print("Error in fetching year")
    ...
    # ID----------------------------------------------
    # try:
    #     if len(Matched_data['27'])and len(Matched_data['27'][0]):
    #         information['ID'] = Matched_data['27'][0][1]
    #         # pages_elem = ET.SubElement(entry_elem, 'pages')
    #         # pages_elem.text = information['pages']
    # except Exception as e:
    #     print("Error in fetching ID")
...
Patterns = { 
    ...
    'Physics_Author_1' : r''' /(\\bibfnamefont {([^}]*)).*(\\bibnamefont {([^}]*))''',
    'Physics_Title_1' : r'''/{title}{([\w\d\s:{},.";'“”-]*)}''',
    'Physics_Journal_1' : r'''\\bibinfo {journal} {([^}]*)''',
    'Physics_Volume_1' : r'''volume} {(\d+)}''',
    'Physics_Pages_1' : r'''{pages} {(\d+)}''',
    'Physics_Year_1' : r'''year} {(\d+)}''',
}
...
#directory = './BBL_file/'
root_directory = './Arxiv_files_extracted/Physics/'
root_directory_output = './Arxiv_files_extracted/results/Physics/'
...
# physics pattern -------------------------------------------------------------------------------------------
                        Matched_data['21']=re.findall(Patterns["Physics_Author_1"], data[0])
                        Matched_data['22']=re.findall(Patterns["Physics_Title_1"], data[0])
                        Matched_data['23']=re.findall(Patterns["Physics_Journal_1"], data[0])
                        Matched_data['24']=re.findall(Patterns["Physics_Volume_1"], data[0])
                        Matched_data['25']=re.findall(Patterns["Physics_Pages_1"], data[0])
                        Matched_data['26']=re.findall(Patterns["Physics_Year_1"], data[0])
#-------------------------------------------------------------------------------------------------------------
                    ...
                    elif(len(Matched_data['22'])):
                        paper_format = "Physics"
                        information = get_information4(root, Matched_data, output_directory + paper_name, paper_format)
                        print("============================Physics=======================================================")
                        print(information)  
                        print("========================================================================================")
            ...
            print(filename, "Done")
            print("::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::")
            # print("Number of paper processed: :")
            # print(count)
